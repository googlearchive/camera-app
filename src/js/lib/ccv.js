// Cache workers to avoid creating them for each invocation.
var workers = [];

// Cache the cascade within the workers for speed.
var cached_cascade = null;
        ...
        return function (complete, error) {
          ...
          for (i in scope.shared) {
            ...
            /* delete DOM object, if any */
              ...
              msg_shared[i] = scope.shared[i];
          }
          for (i = 0; i < worker_num; i++) {
            if (!(i in workers))
              workers[i] = new Worker(file);
            else
              msg_shared.cascade = null;  // Reuse the previous one.
...
var ccv = {
  ...
  detect_objects : parallable("../js/lib/ccv.js", function (canvas, cascade, interval, min_neighbors) {
    ...
    function core(pyr, id, worker_num) {
      ...
      if (cascade)
        cached_cascade = cascade;
      else
        cascade = cached_cascade;
      var interval = this.shared.interval;
      var scale = this.shared.scale;
      var next = this.shared.next;
      var scale_upto = this.shared.scale_upto;
